...
- (void)drawRect:(CGRect)rect {
    ...
    if ([[[NSUserDefaults standardUserDefaults] valueForKey:@"Hud_bgHide"] isEqualToString:@"HUD_bg"]) {
        boxRect = CGRectMake(roundf((allRect.size.width - size.width) / 2) + self.xOffset, roundf((allRect.size.height - size.height) / 2) + 0, 0, 0);
        
    }
    else
    {
      boxRect = CGRectMake(roundf((allRect.size.width - size.width) / 2) + self.xOffset, roundf((allRect.size.height - size.height) / 2) + self.yOffset, size.width, size.height);
    }
